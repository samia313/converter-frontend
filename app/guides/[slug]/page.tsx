      ...
      <article className="max-w-4xl mx-auto px-4 py-12">
        ...
        {relatedGuides.length > 0 && <section className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {relatedGuides.map((relatedGuide) => <Link key={relatedGuide.slug} href={`/guides/${relatedGuide.slug}`} className="p-4 bg-muted rounded-lg hover:bg-muted/80 transition-colors"><h3 className="font-semibold text-sm text-foreground mb-2">{relatedGuide.title}</h3><p className="text-xs text-muted-foreground">{relatedGuide.description}</p></Link>)}
          </div>
        </section>}
        <div className="flex justify-between items-center mt-12 pt-8 border-t border-border">
          <Link href="/guides" className="text-blue-600 hover:text-blue-700">← Back to Guides</Link>
          <Link href="/tools" className="text-blue-600 hover:text-blue-700">Browse Tools →</Link>
        </div>
      </article>
    </main>
  );
