    ...
    <main className="min-h-screen bg-background">
      ...
      <article className="mx-auto max-w-4xl px-4 pb-16 pt-8">
        <header className="mb-10">
          <div className="mb-4 flex flex-wrap items-center gap-3 text-sm text-muted-foreground"><span className="rounded-full bg-blue-600/10 px-3 py-1 font-medium text-blue-600">{post.category}</span><span>{post.readTime} min read</span><time dateTime={post.updatedAt}>Updated {new Date(post.updatedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</time></div>
