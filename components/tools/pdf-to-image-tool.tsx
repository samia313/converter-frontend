...
export default function PdfToImageTool() {
  ...
  const [toolName, setToolName] = useState('pdf-to-image');

  useEffect(() => {
    if (typeof window !== 'undefined') {
      const path = window.location.pathname.split('/')[1];
      setToolName(path || 'pdf-to-image');
    }
  }, []);
  ...
  const handleDownload = () => {
    if (downloadUrl && selectedFile) {
      ...
      a.href = downloadUrl;
      a.download = `${selectedFile.name.split('.')[0]}_converted.zip`;
    ...
    <section className="min-h-screen bg-gradient-to-b from-blue-50 to-white py-12 md:py-20">
      <div className="container mx-auto max-w-2xl px-4">
        <div className="text-center mb-12">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">PDF to Image</h1>
          <p className="text-lg text-gray-600">Convert PDF pages to PNG or JPG images</p>
        </div>
