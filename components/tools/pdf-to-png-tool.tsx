...
export default function PDFToPngTool() {
  ...
  const [downloadType, setDownloadType] = useState<'png' | 'zip'>('png');
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/convert/pdf-to-png', { method: 'POST', body: formData });
      if (!response.ok) {
        const data = await response.json().catch(() => null);
        throw new Error(data?.error || 'PDF to PNG conversion failed.');
      }
      const type = response.headers.get('content-type') || '';
      setDownloadType(type.includes('zip') ? 'zip' : 'png');
      setDownloadUrl(URL.createObjectURL(await response.blob()));
    } catch (err) {
      setError(err instanceof Error ? err.message : 'PDF to PNG conversion failed.');
    } finally {
  ...
  const handleDownload = () => {
    ...
    a.download = downloadType === 'zip'
      ? `${selectedFile.name.replace(/\.[^.]+$/, '')}-png.zip`
    ...
    <section className="min-h-screen bg-gradient-to-b from-blue-50 to-white py-12 md:py-20">
      <div className="container mx-auto max-w-2xl px-4">
        <div className="text-center mb-12">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">PDF to PNG Converter</h1>
          <p className="text-lg text-gray-600">Render PDF pages as PNG images</p>
        </div>
        <div className="bg-white rounded-2xl shadow-lg p-8">
          ...
          {!downloadUrl ? (
            ...
            <div className="text-center mt-6">
              <p className="text-green-600 font-semibold mb-4">PNG output created successfully.</p>
              <div className="flex gap-4 justify-center">
                <button onClick={handleDownload} className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-lg"><Download className="w-5 h-5" />Download {downloadType === 'zip' ? 'PNGs' : 'PNG'}</button>
