...
export default function PDFToJpgTool() {
  ...
  const [toolName, setToolName] = useState('pdf-to-jpg');

  useEffect(() => {
    if (typeof window !== 'undefined') {
      const path = window.location.pathname.split('/')[1];
      setToolName(path || 'pdf-to-jpg');
    }
  }, []);
  ...
  const handleDownload = () => {
    if (downloadUrl && selectedFile) {
      ...
      a.href = downloadUrl;
      a.download = `${selectedFile.name.split('.')[0]}_converted.jpg`;
    ...
    <section className="min-h-screen bg-gradient-to-b from-blue-50 to-white py-12 md:py-20">
      <div className="container mx-auto max-w-2xl px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600">Convert PDF pages to JPG images</p>
        </div>
