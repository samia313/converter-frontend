import {
  Merge,
  Scissors,
  RotateCw,
  Trash2,
  Grid3x3,
  Zap,
  FileText,
  Table,
  Presentation,
  Image,
  Code,
  FileDown,
  FileUp,
  Edit3,
  Stamp,
  Hash,
  Eye,
  Crop,
  Lock,
  Unlock,
  PenTool,
  Wrench,
  MessageCircle,
  FileCheck,
  Globe,
  Scan,
  Award,
  Shield,
  DollarSign,
  BookOpen,
  HelpCircle,
  Settings,
  Briefcase,
  Search,
  Wand2,
  Cloud,
  HardDrive,
  Layers,
  BarChart3,
  Loader,
  QrCode,
  RefreshCw,
  Copy,
  Key,
  ImageIcon,
  Zap as ZapIcon,
  Lock as LockIcon,
  Archive,
  Film,
  Volume2,
  FileArchive,
} from 'lucide-react';

export interface Tool {
  id: string;
  name: string;
  description: string;
  category: string;
  icon: any;
  color: string;
  badge?: string;
}

export const tools: Tool[] = [
  // Organize PDF
  {
    id: 'merge',
    name: 'Merge PDF',
    description: 'Combine multiple PDF files into one document',
    category: 'organize',
    icon: Merge,
    color: '#cc0000',
  },
  {
    id: 'split',
    name: 'Split PDF',
    description: 'Extract pages from PDF or split into multiple files',
    category: 'organize',
    icon: Scissors,
    color: '#cc0000',
  },
  {
    id: 'rotate',
    name: 'Rotate PDF',
    description: 'Rotate PDF pages to the orientation you need',
    category: 'organize',
    icon: RotateCw,
    color: '#cc0000',
  },
  {
    id: 'remove-pages',
    name: 'Remove Pages',
    description: 'Delete unwanted pages from your PDF',
    category: 'organize',
    icon: Trash2,
    color: '#cc0000',
  },
  {
    id: 'organize',
    name: 'Organize PDF',
    description: 'Rearrange, delete and rotate PDF pages with ease',
    category: 'organize',
    icon: Grid3x3,
    color: '#cc0000',
  },
  {
    id: 'drag-drop-reorder',
    name: 'Drag & Drop Reorder',
    description: 'Reorder PDF pages with intuitive drag and drop',
    category: 'organize',
    icon: Layers,
    color: '#cc0000',
  },
  {
    id: 'scan-to-pdf',
    name: 'Scan to PDF',
    description: 'Convert scanned documents to PDF format',
    category: 'organize',
    icon: Scan,
    color: '#cc0000',
  },

  // Optimize
  {
    id: 'compress',
    name: 'Compress PDF',
    description: 'Reduce file size while maintaining quality',
    category: 'optimize',
    icon: Zap,
    color: '#ff9900',
  },

  // Convert to PDF
  {
    id: 'word-to-pdf',
    name: 'Word to PDF',
    description: 'Convert Word documents to PDF format',
    category: 'convert_to',
    icon: FileText,
    color: '#0066cc',
  },
  {
    id: 'excel-to-pdf',
    name: 'Excel to PDF',
    description: 'Convert Excel spreadsheets to PDF',
    category: 'convert_to',
    icon: Table,
    color: '#0066cc',
  },
  {
    id: 'ppt-to-pdf',
    name: 'PowerPoint to PDF',
    description: 'Convert PowerPoint presentations to PDF',
    category: 'convert_to',
    icon: Presentation,
    color: '#0066cc',
  },
  {
    id: 'jpg-to-pdf',
    name: 'JPG to PDF',
    description: 'Convert images to PDF documents',
    category: 'convert_to',
    icon: Image,
    color: '#0066cc',
  },
  {
    id: 'html-to-pdf',
    name: 'HTML to PDF',
    description: 'Convert web pages to PDF',
    category: 'convert_to',
    icon: Code,
    color: '#0066cc',
  },
  {
    id: 'epub-to-pdf',
    name: 'EPUB to PDF',
    description: 'Convert EPUB ebooks to PDF format',
    category: 'convert_to',
    icon: BookOpen,
    color: '#0066cc',
  },
  {
    id: 'cad-to-pdf',
    name: 'CAD to PDF',
    description: 'Convert CAD drawings to PDF documents',
    category: 'convert_to',
    icon: Grid3x3,
    color: '#0066cc',
  },

  // Convert from PDF
  {
    id: 'pdf-to-word',
    name: 'PDF to Word',
    description: 'Extract text from PDF to editable Word documents',
    category: 'convert_from',
    icon: FileDown,
    color: '#00aa00',
  },
  {
    id: 'pdf-to-excel',
    name: 'PDF to Excel',
    description: 'Extract data from PDF to Excel spreadsheets',
    category: 'convert_from',
    icon: FileDown,
    color: '#00aa00',
  },
  {
    id: 'pdf-to-ppt',
    name: 'PDF to PowerPoint',
    description: 'Convert PDF to editable presentations',
    category: 'convert_from',
    icon: FileDown,
    color: '#00aa00',
  },
  {
    id: 'pdf-to-jpg',
    name: 'PDF to JPG',
    description: 'Convert PDF pages to JPG images',
    category: 'convert_from',
    icon: FileDown,
    color: '#00aa00',
  },
  {
    id: 'pdf-to-png',
    name: 'PDF to PNG',
    description: 'Convert PDF pages to PNG images',
    category: 'convert_from',
    icon: FileDown,
    color: '#00aa00',
  },
  {
    id: 'pdf-to-html',
    name: 'PDF to HTML',
    description: 'Convert PDF to editable HTML web pages',
    category: 'convert_from',
    icon: Code,
    color: '#00aa00',
  },

  // Edit & Manage
  {
    id: 'edit-pdf',
    name: 'Edit PDF',
    description: 'Add text, images, shapes to your PDF',
    category: 'edit',
    icon: Edit3,
    color: '#9933cc',
  },
  {
    id: 'watermark',
    name: 'Watermark PDF',
    description: 'Stamp an image or text on your PDF',
    category: 'edit',
    icon: Stamp,
    color: '#9933cc',
  },
  {
    id: 'page-numbers',
    name: 'Page Numbers',
    description: 'Add page numbers to your PDF document',
    category: 'edit',
    icon: Hash,
    color: '#9933cc',
  },
  {
    id: 'redact',
    name: 'Redact PDF',
    description: 'Redact text and graphics to permanently remove',
    category: 'edit',
    icon: Eye,
    color: '#9933cc',
  },
  {
    id: 'crop',
    name: 'Crop PDF',
    description: 'Remove margins and crop PDF pages to exact size',
    category: 'edit',
    icon: Crop,
    color: '#9933cc',
  },
  {
    id: 'protect',
    name: 'Protect PDF',
    description: 'Encrypt PDF with a password',
    category: 'edit',
    icon: Lock,
    color: '#9933cc',
  },
  {
    id: 'unlock',
    name: 'Unlock PDF',
    description: 'Remove password from protected PDF',
    category: 'edit',
    icon: Unlock,
    color: '#9933cc',
  },
  {
    id: 'sign',
    name: 'Sign PDF',
    description: 'Add your signature to PDF documents',
    category: 'edit',
    icon: PenTool,
    color: '#9933cc',
  },
  {
    id: 'password-recovery',
    name: 'Password Recovery',
    description: 'Recover lost PDF passwords and decrypt files',
    category: 'edit',
    icon: Key,
    color: '#9933cc',
  },
  {
    id: 'pdf-repair',
    name: 'PDF Repair',
    description: 'Fix corrupted PDF files and recover data',
    category: 'edit',
    icon: Wrench,
    color: '#9933cc',
  },
  {
    id: 'compare-pdf',
    name: 'Compare PDFs',
    description: 'Find differences between two PDF documents',
    category: 'edit',
    icon: Copy,
    color: '#9933cc',
  },

  // AI Features
  {
    id: 'chat-pdf',
    name: 'Chat With PDF',
    description: 'Ask questions about your documents with AI',
    category: 'ai',
    icon: MessageCircle,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'summary',
    name: 'AI PDF Summary',
    description: 'Generate intelligent summaries of long PDF documents',
    category: 'ai',
    icon: Zap,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'chat',
    name: 'AI Chat PDF',
    description: 'Ask intelligent questions about PDF documents using AI',
    category: 'ai',
    icon: MessageCircle,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'translate',
    name: 'AI Translation',
    description: 'Translate PDF documents and text to 100+ languages',
    category: 'ai',
    icon: Globe,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'translate',
    name: 'AI Translate PDF',
    description: 'Translate entire PDF to 10+ languages instantly',
    category: 'ai',
    icon: Globe,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'rewrite',
    name: 'AI Rewrite PDF',
    description: 'Rewrite and improve PDF content with AI suggestions',
    category: 'ai',
    icon: Wand2,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'research-assistant',
    name: 'AI Research Assistant',
    description: 'Extract insights and generate research summaries',
    category: 'ai',
    icon: Search,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'ocr',
    name: 'AI OCR',
    description: 'Extract text from scanned PDFs and images',
    category: 'ai',
    icon: Scan,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'resume-builder',
    name: 'AI Resume Builder',
    description: 'Create professional resumes with AI assistance',
    category: 'ai',
    icon: Award,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'contract-analyzer',
    name: 'AI Contract Analyzer',
    description: 'Extract clauses, analyze risks, and understand contracts',
    category: 'ai',
    icon: Shield,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'invoice',
    name: 'AI Invoice',
    description: 'Generate professional invoices with AI assistance',
    category: 'ai',
    icon: DollarSign,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'cover-letter',
    name: 'AI Cover Letter Generator',
    description: 'Generate personalized cover letters instantly',
    category: 'ai',
    icon: FileText,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'business-proposal',
    name: 'AI Business Proposal',
    description: 'Create professional business proposals quickly',
    category: 'ai',
    icon: Briefcase,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'study-notes',
    name: 'AI Notes Generator',
    description: 'Convert PDFs into organized study notes automatically',
    category: 'ai',
    icon: BookOpen,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'quiz-generator',
    name: 'AI Quiz Generator',
    description: 'Create multiple choice questions from PDFs automatically',
    category: 'ai',
    icon: HelpCircle,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'table-extraction',
    name: 'AI Table Extraction',
    description: 'Extract and convert tables from PDFs to Excel',
    category: 'ai',
    icon: Table,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'metadata',
    name: 'PDF Metadata Editor',
    description: 'Edit PDF title, author, and keywords',
    category: 'ai',
    icon: Settings,
    color: '#cc00cc',
  },
  {
    id: 'batch-conversion',
    name: 'Batch Conversion',
    description: 'Convert multiple files at once automatically',
    category: 'ai',
    icon: Layers,
    color: '#cc00cc',
    badge: 'NEW',
  },
  {
    id: 'qr-code-generator',
    name: 'QR Code Generator',
    description: 'Generate QR codes and embed them in PDFs',
    category: 'ai',
    icon: QrCode,
    color: '#cc00cc',
    badge: 'NEW',
  },
  
  // Cloud Storage & Integration
  {
    id: 'cloud-storage',
    name: 'Cloud Storage',
    description: 'Save and sync PDFs across multiple clouds',
    category: 'organize',
    icon: Cloud,
    color: '#cc0000',
    badge: 'NEW',
  },
  {
    id: 'google-drive',
    name: 'Google Drive',
    description: 'Direct integration with Google Drive',
    category: 'organize',
    icon: HardDrive,
    color: '#cc0000',
    badge: 'NEW',
  },
  {
    id: 'dropbox',
    name: 'Dropbox',
    description: 'Direct integration with Dropbox cloud storage',
    category: 'organize',
    icon: HardDrive,
    color: '#cc0000',
    badge: 'NEW',
  },
  {
    id: 'onedrive',
    name: 'OneDrive',
    description: 'Direct integration with Microsoft OneDrive',
    category: 'organize',
    icon: HardDrive,
    color: '#cc0000',
    badge: 'NEW',
  },

  // Utility Tools
  {
    id: 'image-compressor',
    name: 'Image Compressor',
    description: 'Compress images to reduce file size without losing quality',
    category: 'optimize',
    icon: ImageIcon,
    color: '#ff8800',
    badge: 'NEW',
  },
  {
    id: 'image-converter',
    name: 'Image Converter',
    description: 'Convert images between PNG, JPG, WebP, GIF and more',
    category: 'convert_to',
    icon: ImageIcon,
    color: '#0066cc',
    badge: 'NEW',
  },
  {
    id: 'password-generator',
    name: 'Password Generator',
    description: 'Generate secure, random passwords for your accounts',
    category: 'security',
    icon: LockIcon,
    color: '#ff0000',
    badge: 'NEW',
  },
  {
    id: 'file-converter',
    name: 'File Converter',
    description: 'Convert files between multiple formats quickly',
    category: 'convert_to',
    icon: RefreshCw,
    color: '#0066cc',
    badge: 'NEW',
  },
  {
    id: 'zip-extractor',
    name: 'ZIP Extractor',
    description: 'Extract and manage ZIP, RAR, 7Z archives easily',
    category: 'organize',
    icon: Archive,
    color: '#cc0000',
    badge: 'NEW',
  },
  {
    id: 'video-converter',
    name: 'Video Converter',
    description: 'Convert videos to MP4, WebM, AVI, MOV and other formats',
    category: 'convert_to',
    icon: Film,
    color: '#0066cc',
    badge: 'NEW',
  },
  {
    id: 'audio-converter',
    name: 'Audio Converter',
    description: 'Convert audio files to MP3, WAV, FLAC, AAC and more',
    category: 'convert_to',
    icon: Volume2,
    color: '#0066cc',
    badge: 'NEW',
  },
  {
    id: 'ocr-text',
    name: 'OCR',
    description: 'Extract text from images and scanned documents',
    category: 'ai',
    icon: Scan,
    color: '#cc00cc',
    badge: 'NEW',
  },
];

export const toolsByCategory = {
  organize: tools.filter(t => t.category === 'organize'),
  optimize: tools.filter(t => t.category === 'optimize'),
  convert_to: tools.filter(t => t.category === 'convert_to'),
  convert_from: tools.filter(t => t.category === 'convert_from'),
  edit: tools.filter(t => t.category === 'edit'),
  ai: tools.filter(t => t.category === 'ai'),
};
